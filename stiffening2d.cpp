...
int main(int argc, char **argv)
{
    ...
    const auto getJ = [&reference, &m](const std::vector<double> &X, int t) -> mat<2, 2>
    { // get Jacobian matrix for triangle t
        mat<2, 2> J = {};
        for (int i : {0, 1, 2})
            for (int d : {0, 1})
...
#pragma omp parallel for reduction(max : t)
    ...
    std::cerr << "Stiffening " << argv[1] << ", nverts: " << m.nverts() << ", " << " max distortion: " << qual_max(X) << std::endl;
...
#pragma omp parallel for reduction(+ : F) reduction(+ : err)
        ...
        if (std::abs(qual_max_prev - qmax) / qmax < outer_threshold)
            break;
